...
Handle  g_hCooldown;  /**< Console Variable */
Handle  g_hReasons;   /**< ArrayList */
...
public void OnPluginStart() {
  ...
  AutoExecConfig(true, "report", "discord");
}
...
public void OnCooldownUpdated(Handle hConVar, const char[] szOV, const char[] szNV) {
  g_iCooldown = GetConVarInt(g_hCooldown);
}
...
public Action OnSayHook(int iClient, int iArgs) {
  ...
  // crutch for Team Fortress 2, which all message encloses in quotation marks, because of what one argument comes out.
  if (iArgs == 1)
